-- The payment-review cron (app/api/cron/check-payment-reviews) writes audit-log
-- events for flagging activity, but the crypto_audit_log event_type CHECK
-- constraint did not include them — so those inserts silently failed. Add the
-- three review event types so the audit trail actually records flagging.
