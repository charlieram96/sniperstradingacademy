...
export function AccountStatusCard({
  ...
  const getStatusColor = () => {
    ...
    if (bypassSubscription) return "bg-gold-400/10 text-gold-400 border-gold-400/20"
    ...
    return "bg-gold-400/10 text-gold-400 border-gold-400/20"
  }

  const getStatusIcon = () => {
    // Bypassed users always show checkmark
    if (bypassSubscription) return <CheckCircle className="h-5 w-5 text-gold-400" />
    ...
    return <CheckCircle className="h-5 w-5 text-gold-400" />
  }
